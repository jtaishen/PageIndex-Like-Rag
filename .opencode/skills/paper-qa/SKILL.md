...
7. 对普通内容问答，先调用 `kb_classify_query`，再调用 `kb_tree_search` 定位章节、段落或图表节点；如果只需要旧版扁平节点结果，再调用 `kb_search_tree`。
8. 调用 `kb_get_evidence` 获取 evidence packet。
9. 对跨论文比较，调用 `kb_compare` 并读取比较矩阵中的 evidence。
10. 对综述草稿，调用 `kb_generate_review`、`kb_draft_review`、`kb_check_review_citations`，必要时再 `kb_assemble_review`。
11. 仅基于 evidence packet、事实层 evidence ID、论文理解工件或任务工件中的 evidence 字段回答。
12. 若证据不足，明确说明不足，不要补写没有来源的结论。
13. 如果用户要求复盘质量，调用 `kb_get_query_stats` 或 `kb_eval_search`；如果是综述任务，调用 `kb_eval_review`；如果复盘事实层或表格指标可靠性，调用 `kb_eval_facts` 或 `kb_audit_facts`。
14. 如果用户指出某次结果正确、错误、证据不足或检索遗漏，调用 `kb_put_feedback` 记录评分、标签、期望 doc/node/keyword 和短评论。
15. 需要沉淀评测集时，调用 `kb_build_eval_set_from_feedback`，再用 `kb_eval_search` 比较 `hybrid/tree/fts`。
16. 需要复盘趋势时，调用 `kb_eval_dashboard`，只使用其中的统计、warning 和建议动作。
17. 需要调优检索策略时，调用 `kb_tune_search`，保存 profile 后再调用 `kb_apply_search_profile`；只有用户要求或任务明确需要时，才传入 `search_mode="auto"`。
18. 需要验证 PageIndex-like 树检索是否优于扁平检索时，调用 `kb_create_eval_suite`、`kb_run_benchmark`、`kb_analyze_failures` 和 `kb_generate_case_study`；报告只用于复盘，不作为论文内容证据。
