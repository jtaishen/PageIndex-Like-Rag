...
7. 如果后续需要结构化事实层，先调用 `kb_extract_doc_insights`，再调用 `kb_extract_facts` 生成 claims/entities/relations；涉及实验表格时追加读取 `kb_get_table_content` 和 `kb_get_table_summaries`。
...
- v0.16 会在 v0.14 的 `layout_blocks.json`、`figures.json`、`tables.json` 和 `reference_sections.json` 基础上追加 `table_content.json` 与 `table_summaries.json`；需要检查实验指标或表格结论时优先读取 `kb_get_tables`、`kb_get_table_content` 和 `kb_get_table_summaries`。
