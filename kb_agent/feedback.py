...
def _dashboard_markdown(dashboard: Dict[str, Any]) -> str:
    ...
    lines = [
        ...
        f"- latest_claim_graph_id: `{graph.get('graph_id', '')}`",
        ...
        f"- latest_removed_paragraph_count: `{baseline.get('removed_paragraph_count', 0)}`",
    ...
    lines.extend(["", "## Claim Graphs"])
    for report in dashboard.get("latest_claim_graphs") or []:
        lines.append(
            f"- graph=`{report.get('graph_id')}` conflicts=`{report.get('conflict_count')}` "
...
def _dashboard_html(dashboard: Dict[str, Any]) -> str:
    ...
    graph_html = _list_section(
        "Claim Graphs",
        [
            (
                f"graph={item.get('graph_id', '')} conflicts={item.get('conflict_count', 0)} "
                f"isolated={item.get('isolated_fact_count', 0)} coverage={item.get('evidence_coverage_rate', 0.0)} "
    ...
    baseline_html = _list_section(
        ...
        [
            ...
            f"stale={baseline.get('baseline_stale_reason', '')} path={baseline.get('path', '')}",
            f"review_draft={baseline.get('review_draft_status', '')} quality={baseline.get('review_draft_quality_level', '')} "
            f"skipped={baseline.get('skipped_section_count', 0)} removed={baseline.get('removed_paragraph_count', 0)} "
