...
def baseline_markdown(report: Dict[str, Any]) -> str:
    ...
    lines = [
        ...
        f"- negative_memory_count: `{(report.get('memory_context') or {}).get('negative_memory_count', 0)}`",
        ...
        f"- memory_plan_available: `{(report.get('memory_context') or {}).get('memory_plan_available', False)}`",
...
def baseline_html(report: Dict[str, Any]) -> str:
    ...
    cards = [
        ...
        ("Negative Memory", memory_context.get("negative_memory_count", 0)),
        ("Artifact Refs", memory_context.get("artifact_ref_count", 0)),
        ("Memory Plan", memory_context.get("memory_plan_available", False)),
